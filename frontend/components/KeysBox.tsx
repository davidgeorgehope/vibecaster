...
import { Key, Plus, Trash2, Copy, Check, Loader2, AlertCircle, Eye, EyeOff, Terminal } from 'lucide-react';

interface KeysBoxProps {
  token: string | null;
...
export default function KeysBox({ token, showNotification }: KeysBoxProps) {
  ...
  return (
    <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-6">
      ...
      <p className="text-sm text-gray-400 mb-4">
        API keys authenticate CLI and agent requests. Create a key, then use it with <code className="text-purple-300 bg-gray-800 px-1 rounded">vibecaster login</code>.
      </p>
      ...
      {keys.length === 0 ? (
        <div className="text-center py-6 text-gray-500">
          ...
          <p>No API keys yet</p>
          <p className="text-sm mt-1">Create one to use the CLI or agent skill</p>
        </div>
      ) : (
      ...
      <div className="mt-6 pt-6 border-t border-gray-700">
        <h3 className="text-sm font-semibold text-purple-400 uppercase tracking-wide mb-2">API Authentication</h3>
