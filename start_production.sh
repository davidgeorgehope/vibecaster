...
# Otherwise, use localhost URL for direct access
if [ "$NGINX_CONFIGURED" = true ]; then
    cat > "$FRONTEND_DIR/.env.local" <<EOF
# Auto-generated by start_production.sh
# Using relative URL - nginx will proxy /api to backend
NEXT_PUBLIC_API_URL=/api
EOF
    echo -e "   ✅ Frontend configured to use /api (nginx will proxy to backend)"
else
    cat > "$FRONTEND_DIR/.env.local" <<EOF
# Auto-generated by start_production.sh
